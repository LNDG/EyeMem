...
bsYA = reshape(result.usc(1:nsub(1)*ncond,1), [nsub(1) 5]);
bsOA = reshape(result.usc(nsub(1)*ncond+1:sum(nsub)*ncond,1), [nsub(2) 5]);
...
rm = fitrm(t,'bin1-bin5~agegroup ','WithinDesign',Meas)
ranova(rm)
