function EM_pls_BOLDvsdprime(cfg)
...
disp(sourcefile)
load(sourcefile); % source_std comes out
source=source_std; clear source_std
...
load('/Users/kloosterman/gridmaster2012/kloosterman/projectdata/eyemem/variability/ftsource/common_coords.mat');
...
%   case 'taskPLS'
    ...
    tmp.session_info.datamat_prefix = 'BOLDvsDprime'; %[subj '_' pattern];%stores common      %datamat prefix
    tmp.behavdata = [];
...
tmp.session_info.pls_data_path = fileparts(outfile_sesdat); %this will add info to 'a' for   tracking purposes, and for commands below.
...
for i = 1:tmp.session_info.num_conditions
  tmp.session_info.condition{end+1} = sprintf('cond%d', i); % OA vs YA??
end
