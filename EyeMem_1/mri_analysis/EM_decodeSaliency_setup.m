function [accuracy] = EM_decodeSaliency_setup()
% run from runMIBmeg_analysis
% Decode HMAX bin from fMRI data

if ismac
  basepath = '/Users/kloosterman/projectdata/eyemem/'; %yesno or 2afc
  %     backend = 'parfor';
  backend = 'local';
  %   backend = 'qsublocal';
  compile = 'no';
...
PREIN = fullfile(basepath, 'variability2', '5TRspertrial', 'ftsource');
PREOUT =  fullfile(basepath, 'variability2', '5TRspertrial', 'ftsource', 'decoding');
...
% agedirs = {'YA' 'OA'};
agedirs = {''};
outname = 'decode'; % decode HMAX from single trial SD
mkdir(fullfile(PREOUT)) % TODO age groups
mkdir(fullfile(PREOUT))
