...
for SUB in ${SubjectID} ; do
		...
		echo "sleep $(( RANDOM % 120 ))"					>> job.slurm # Sleep for a random period between 1-60 seconds, used to avoid interference when running antsBrainExtraction.sh
		
    	echo "module load ants" 						>> job.slurm
		
		echo "cd ${ANTsPath}"								>> job.slurm
		echo "echo 'ANTs will start now' >> ${StartLog}"	>> job.slurm
		
		# Perform Brain Extraction
		
		echo -n "${ScriptsPath}/antsBrainExtraction_cpgeom.sh -d ${ImageDimension} -a ${AnatPath}/${AnatImage}.nii.gz -e ${TemplateImage} " 	>> job.slurm
		echo  "-m ${ProbabilityImage} -f ${RegistrationMask} -k ${KeepTemporaryFiles} -q 1 -o ${ANTsPath}/${ANTsName}" 							>> job.slurm
		...
		echo "mv ${ANTsPath}/${ANTsName}BrainExtractionBrain.nii.gz ${ANTsPath}/${SUB}_T1w_brain.nii.gz" >> job.slurm
		
		echo "rm ${StartLog}" >> job.slurm
