      ...
      if [[ ! -f ${EXTRACTION_SEGMENTATION} ]]; 
        ...
        for (( i = 0; i < ${#ANATOMICAL_IMAGES[@]}; i++ ))
          do
			...
			# FIX: copy headers geometric information
			fslcpgeom ${ANATOMICAL_IMAGES[$i]} ${N4_CORRECTED_IMAGES[$i]} -d
			fslcpgeom ${ANATOMICAL_IMAGES[$i]} ${EXTRACTION_MASK} -d
		
			echo "${ANATOMICAL_IMAGES[$i]} header copied to ${N4_CORRECTED_IMAGES[$i]} and ${EXTRACTION_MASK}"
